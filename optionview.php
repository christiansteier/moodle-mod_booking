...
// Falls der Autoloader die Klasse nicht findet, manuell einbinden.
require_once($CFG->dirroot . '/mod/booking/classes/singleton_service.php');

global $DB, $PAGE, $OUTPUT, $USER;

// Erzwinge den Login, falls der Benutzer nicht eingeloggt ist.
if (!isloggedin()) {
    redirect(new moodle_url('/login/index.php', array('redirect' => $PAGE->url)));
}
...
// Falls der Namespace nicht korrekt funktioniert, direkt verwenden:
$booking = \mod_booking\singleton_service::get_instance_of_booking_by_cmid($cmid);
...
if (!$cm->uservisible) {
    echo $OUTPUT->header();
    echo html_writer::div(get_string('invisibleoption:notallowed', 'mod_booking'), "alert alert-danger");
    echo $OUTPUT->footer();
    die();
}

if ($settings = \mod_booking\singleton_service::get_instance_of_booking_option_settings($optionid)) {

    if ($userid == $USER->id || $userid == 0) {
        $user = $USER;
    } else {
        $user = \mod_booking\singleton_service::get_instance_of_user($userid);
    }

    $bookinganswer = \mod_booking\singleton_service::get_instance_of_booking_answers($settings);
